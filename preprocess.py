...
def run():
    ...
    for target_file in csv_list:
        ...
        for index, line in enumerate(data_lines):
            ...
            if index != 0:
                ...
                if stock_close < next_stock_close:
                    price_up = 1
                else:
                    price_up = 0
                pre_stock_data.append((stock_high, stock_low, stock_open, stock_close, stock_volume, price_up))
            stock_high = float(line_split[1].strip())
            stock_low = float(line_split[2].strip())
            stock_open = float(line_split[3].strip())
            stock_close = float(line_split[4].strip())
            stock_volume = float(line_split[5].strip())
    ...
    train_save_file = open('preprocessed_data/train_data.csv', 'a')
